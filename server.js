...
// КЛЮЧ В ОДНУ СТРОКУ - НЕ РАЗРЫВАЙ ЕЁ
...
app.post('/upload', async (req, res) => {
    try {
        const { worker, city, client, address, pod, image, coords } = req.body;
        ...
        const name = `${address}_п.${pod}.jpg`.replace(/[/\\?%*:|"<>]/g, '-');
        await drive.files.create({ resource: { name, parents: [f4] }, media: { mimeType: 'image/jpeg', body: Buffer.from(image, 'base64') } });
...
app.post('/check-license', (req, res) => {
    // Временная заглушка, чтобы лицензия всегда была "вечной" пока чиним сервер
