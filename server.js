...
const bot = new TelegramBot(BOT_TOKEN, { polling: false }); // Отключили лишний опрос
...
app.post('/upload', async (req, res) => {
    try {
        ...
        // ТЕЛЕГРАМ ТЕПЕРЬ НЕ ТОРМОЗИТ ЗАГРУЗКУ
        bot.sendMessage(ADMIN_ID, `✅ ПРИНЯТО: ${address}\n📍 ${coords}`).catch(() => console.log("ТГ уведомление не ушло, но фото на диске!"));
...
app.listen(process.env.PORT || 3000, () => console.log("--- СИСТЕМА ПЕРЕЗАПУЩЕНА ---"));
