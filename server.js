...
app.get('/dashboard', (req, res) => {
    ...
    <style>
        ...
        .btn-del{background:#d63031;color:#fff;font-weight:bold;cursor:pointer;border:none;margin-top:5px;padding:8px}
        .status-ok{color:#4cd137} .status-expired{color:#e84118}
        code{background:#000;padding:2px 5px;border-radius:4px;color:#f0ad4e}
    </style></head><body>
    ...
    <div class="card">
        ...
        <select id="d">
            ...
            <option value="180">На 180 дней (Полгода)</option>
            <option value="365">На 1 год</option>
        </select>
        <button class="btn-main" onclick="add()">СОЗДАТЬ ОБЪЕКТ</button>
    </div>
    <div id="list">Загрузка...</div>
    <script>
        const PASS="${ADMIN_PASS}";
        function auth(){if(localStorage.getItem('p')!==PASS){let p=prompt('PASS');if(p===PASS)localStorage.setItem('p',PASS);else auth();}}
        async function load(){
            ...
            document.getElementById('list').innerHTML=d.map(k=> {
                ...
                return '<div class="card">' +
                    '<b>' + k.name + '</b><br>' +
                    '<code>' + k.key + '</code><br>' +
                    'До: <span class="' + (isExp?'status-expired':'status-ok') + '">' + new Date(k.expiry).toLocaleDateString() + '</span><br>' +
                    'Клиент ID: ' + (k.ownerChatId || 'Свободен') +
                    ...
                    '<button class="btn-del" onclick="del(\\''+k.key+'\\')">УДАЛИТЬ КЛЮЧ</button>' +
                '</div>'
            }).join('');
        }
        async function add(){
        ...
        async function del(key){
            if(confirm('ВНИМАНИЕ: Удалить ключ ' + key + ' навсегда?')){
...
bot.start(async (ctx) => {
    ...
    if (cid === MY_TELEGRAM_ID) {
        return ctx.reply('👑 ПАНЕЛЬ РАЗРАБОТЧИКА', { reply_markup: { inline_keyboard: [[{ text: "📦 УПРАВЛЕНИЕ СИСТЕМОЙ", web_app: { url: SERVER_URL + "/dashboard" } }]] } });
    }
    const keys = await readDatabase(); 
    if (keys.find(k => String(k.ownerChatId) === String(cid))) {
        return ctx.reply('🏢 ЛИЧНЫЙ КАБИНЕТ КЛИЕНТА', { reply_markup: { inline_keyboard: [[{ text: "📊 МОИ ОБЪЕКТЫ", web_app: { url: SERVER_URL + "/client-panel?chatId=" + cid } }]] } });
    }
...
bot.on('text', async (ctx) => {
    ...
    if (idx !== -1) { 
        ...
        await saveDatabase(keys); 
        ctx.reply('✅ Доступ открыт!', { reply_markup: { inline_keyboard: [[{ text: "📊 ПАНЕЛЬ УПРАВЛЕНИЯ", web_app: { url: SERVER_URL + "/client-panel?chatId=" + ctx.chat.id } }]] } });
