...
const app = express();
app.use(cors());
// Увеличены лимиты для работы с HD фото и PDF отчетами
...
// --- ФУНКЦИЯ ДЛЯ МЕРЧАНДАЙЗИНГА (ОБНОВЛЕННАЯ АНАЛИТИКА) ---
...
app.post('/upload', async (req, res) => {
    try {
        ...
        await appendToReport(workerId, worker, city, todayStr, address, entrance || "-", finalFolderName, workType || "Не указан", price || 0, lat, lon);
...
app.post('/api/keys/extend', async (req, res) => {
    ...
    if (idx !== -1) {
        let d = new Date(keys[idx].expiry); d.setDate(d.getDate() + 30);
        keys[idx].expiry = d.toISOString(); await saveDatabase(keys); res.json({ success: true });
