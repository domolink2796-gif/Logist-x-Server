...
app.post('/upload', async (req, res) => {
    try {
        ...
        // Упрощенная логика папок
        const folder = await drive.files.create({
            resource: { name: client || "Объект", mimeType: 'application/vnd.google-apps.folder', parents: [MY_ROOT_ID] },
            fields: 'id'
        });
        if (image) {
            const buffer = Buffer.from(image, 'base64');
            await drive.files.create({
                resource: { name: `${address}.jpg`, parents: [folder.data.id] },
...
app.get('/', (req, res) => res.send("СИСТЕМА ЛОГИСТИКА X ЗАПУЩЕНА"));

app.listen(process.env.PORT || 3000, () => console.log("СЕРВЕР РАБОТАЕТ"));
