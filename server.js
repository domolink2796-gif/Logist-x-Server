...
// --- ФУНКЦИИ РАБОТЫ С DRIVE И ТАБЛИЦАМИ (БЕЗ УПРОЩЕНИЙ) ---
...
async function saveDatabase(keys) {
    ...
    } catch (e) { console.error("Database save error"); }
}

async function getOrCreateFolder(name, parentId) {
    const cleanName = String(name).trim();
    const q = `name = '${cleanName.replace(/'/g, "\\'")}' and mimeType = 'application/vnd.google-apps.folder' and '${parentId}' in parents and trashed = false`;
    const res = await drive.files.list({ q });
    if (res.data.files.length > 0) return res.data.files[0].id;
    const file = await drive.files.create({ resource: { name: cleanName, mimeType: 'application/vnd.google-apps.folder', parents: [parentId] }, fields: 'id' });
    return file.data.id;
}
...
async function appendToReport(spreadsheetId, data) {
    ...
    } catch (e) { console.error("Sheets Append Error"); }
}

// --- ОСНОВНОЙ РОУТ /UPLOAD (ЛОГИСТ) ---
app.post('/upload', async (req, res) => {
    try {
        ...
        // 2. ЗАГРУЗКА ФОТО И ЗАПИСЬ В ТАБЛИЦУ (ТВОЯ РОДНАЯ ЛОГИКА)
        if (image) {
            const workerFolderId = await getOrCreateFolder(workerName, MY_ROOT_ID);
            const cleanAddr = String(address).replace(/[\\\/\?\*\:\|\"<>]/g, "");
            ...
            const file = await drive.files.create({
                resource: { name: fileName, parents: [workerFolderId] },
                media: media,
                fields: 'id, webViewLink'
            });

            const tableId = '1f1S797i037tVIdy40G9o0Y5I7GfR00u6O_y92G5H2E0'; 
            await appendToReport(tableId, {
                date: date,
                address: address,
                entrance: entrance,
                worker: workerName,
                status: status,
                photoUrl: file.data.webViewLink
            });

            return res.json({ success: true, url: file.data.webViewLink });
        }

        res.json({ success: false, message: "No image or action provided" });
    } catch (e) {
...
app.post('/merch-upload', async (req, res) => {
    try {
        ...
        await sheets.spreadsheets.values.append({
            spreadsheetId: '1CuCMuvL3-tUDoE8UtlJyWRyqSjS3Za9p', 
