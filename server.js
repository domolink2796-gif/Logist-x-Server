...
// --- БД Штрих-кодов (Локальная для каждого ключа) ---
...
// ПОЛУЧЕНИЕ ВСЕГО КАТАЛОГА (Для синхронизации всех мерчей под одним ключом)
...
app.post('/save-product', async (req, res) => { // Псевдоним для совместимости с фронтендом
    try {
    ...
    <script>
        ...
        async function load(){
            ...
            document.getElementById('root').innerHTML = keys.map(k => {
                ...
                return \`
                <div class="card">
                    <div class="status-badge">\text{\${days > 0 ? 'Доступ активен' : 'Срок истек'}}</div>
                    <div class="obj-name">\text{\${k.name} (\${k.type || 'logist'})}</div>
                    <div style="font-size: 11px; opacity: 0.4; margin-bottom: 15px;">Ключ: \text{\${k.key}}</div>
                    <div class="warning-box">⚠️ ФОТО-ОТЧЕТЫ И АРХИВЫ ХРАНЯТСЯ 60 ДНЕЙ.<br><b>СОХРАНЯЙТЕ ДАННЫЕ ВОВРЕМЯ!</b></div>
                    <div class="stats">
                        <div class="stat-item"><span class="stat-val">\text{\${days > 0 ? days : 0}}</span><span class="stat-lbl">Дней</span></div>
                        <div class="stat-item"><span class="stat-val">\text{\${k.workers.length}/\${k.limit}}</span><span class="stat-lbl">Людей</span></div>
                    </div>
                    <div class="workers-box">\text{\${workersList.join('')}}</div>
                    <div class="grid-prices">
                        <div class="price-card" onclick="req('\text{\${k.key}}','\text{\${k.name}}',30,'\text{\${k.type}}')">
                            <div style="font-size:14px; font-weight:800">30 дн.</div>
                            <div style="font-size:10px; color:#f59e0b">\text{\${k.limit*1500}}₽</div>
                        </div>
                        <div class="price-card" onclick="req('\text{\${k.key}}','\text{\${k.name}}',90,'\text{\${k.type}}')">
                            <div class="sale-tag">-10%</div>
                            <div style="font-size:14px; font-weight:800">90 дн.</div>
                            <div style="font-size:10px; color:#f59e0b">\text{\${k.limit*4050}}₽</div>
                        </div>
                    </div>
                </div>\`;
            }).join('');
