...
// === ПЕРЕХВАТ ПАРТНЕРА (logist-x.ru/reg?ref=...) ===
app.get('/reg', (req, res) => {
    const ref = req.query.ref || '';
    res.send(`<html><head><title>LOGIST_X</title></head><body style="background:#010409;color:white;display:flex;justify-content:center;align-items:center;height:100vh;font-family:sans-serif;">
...
bot.on('callback_query', async (ctx) => {
    ...
    if (data === 'role_partner') {
        ...
        ctx.reply(`🤝 **ПАРТНЕРСКАЯ ПРОГРАММА LOGIST_X**\n\nТвоя уникальная ссылка для клиентов:\n\`${refLink}\`\n\n📜 **УСЛОВИЯ:**\n1. Платим **15%** с каждой покупки лицензии.\n2. Клиент закрепляется за тобой навсегда.\n3. Выплаты бонусов — по запросу в любое время.`, { parse_mode: 'Markdown' });
    }
});
