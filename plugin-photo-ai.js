module.exports = function(app, context) {
    app.post('/api/photo-ai-process', async (req, res) => {
        console.log("📥 [AI] Запуск (Hugging Face - Бесплатно)...");
        ...
        try {
            // Читаем токен Hugging Face из файла на сервере
            ...
            // Используем стабильную модель Qwen2.5-VL
            ...
            const cmd = `curl -s -X POST https://api-inference.huggingface.co/models/${MODEL}/v1/chat/completions \
            ...
            exec(cmd, { maxBuffer: 1024 * 1024 * 50 }, (error, stdout) => {
                ...
                try {
                    ...
                    if (data.choices && data.choices[0]) {
                        ...
                        // Очистка от markdown (```base64 ...)
                        let finalBase64 = content.replace(/```base64|```|data:image\/\w+;base64,|data:image\/png;base64,/g, '').trim();
                        
                        console.log("✅ [AI] ФОТО ГОТОВО! (Через Hugging Face)");
                        res.json({ success: true, processedImage: "data:image/jpeg;base64," + finalBase64 });
                    } else {
                        console.error("❌ Ответ HF:", stdout);
                        res.status(500).json({ error: "Модель HF занята, попробуйте еще раз через 30 секунд" });
                    }
                } catch (e) {
